    ...
    describe '#system_env' do
      ...
      context 'when there are services' do
        ...
        it 'contains a populated vcap_services' do
          ...
          expect(system_env_presenter.system_env['VCAP_SERVICES']).to have_key(service.label.to_s)
          expect(system_env_presenter.system_env['VCAP_SERVICES'][service.label.to_s]).to have(1).services
        end

        it 'includes service binding information' do
          expect(system_env_presenter.system_env['VCAP_SERVICES'][service.label.to_s]).to have(1).items
          expect(system_env_presenter.system_env['VCAP_SERVICES'][service.label.to_s].first).to eq(
        ...
        describe 'grouping' do
          ...
          it 'should group services by label' do
            ...
            expect(system_env_presenter.system_env['VCAP_SERVICES'][service.label.to_s]).to have(2).services
            expect(system_env_presenter.system_env['VCAP_SERVICES'][service_alt.label.to_s]).to have(1).service
          end
        end
      end
    end
