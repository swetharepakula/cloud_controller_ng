    ...
    describe 'fetching lrp status' do
      context 'when there is a tps url configured' do
        context 'and the first attempt returns lrp status' do
          before do
            stub_request(:get, tps_status_url.to_s).to_return(
              status: 200,
              body: [
                {
                  process_guid: 'abc',
        ...
        context 'when the TPS endpoint is unavailable' do
          it 'retries and eventually raises InstancesUnavailable' do
            stub = stub_request(:get, tps_status_url.to_s).to_raise(Errno::ECONNREFUSED)
        ...
        context 'when the TPS endpoint 404s' do
          before do
            stub_request(:get, tps_status_url.to_s).to_return(status: 404, body: 'Could not find it')
          end
        ...
        context 'when the TPS endpoint fails' do
          before do
            stub_request(:get, tps_status_url.to_s).to_return(status: 500, body: 'This Broke')
          end
    ...
    describe 'fetching lrp stats' do
      context 'when there is a tps stats url is configured' do
        ...
        context 'and the first attempt returns instance info with stats' do
          before do
            stub_request(:get, tps_stats_url.to_s).with(
        ...
        context 'when the TPS endpoint is unavailable' do
          it 'retries and eventually raises InstancesUnavailable' do
            stub = stub_request(:get, tps_stats_url.to_s).to_raise(Errno::ECONNREFUSED)
        ...
        context 'when the TPS endpoint 404s' do
          before do
            stub_request(:get, tps_stats_url.to_s).to_return(status: 404, body: 'Could not find it')
          end
        ...
        context 'when the TPS endpoint fails' do
          before do
            stub_request(:get, tps_stats_url.to_s).to_return(status: 500, body: ' ')
          end
    ...
    describe 'fetching bulk lrp status' do
      context 'when there is a tps url configured' do
        context 'and the first attempt returns' do
          before do
            stub_request(:get, tps_bulk_status_url.to_s).to_return(
              status: 200,
              body: {
        ...
        context 'when an empty array is passed in' do
          ...
          it 'does not make a request' do
            stub = stub_request(:get, tps_bulk_status_url.to_s).to_raise("this shouldn't be called")
        ...
        context 'when the TPS endpoint is unavailable' do
          it 'retries and eventually raises InstancesUnavailable' do
            stub = stub_request(:get, tps_bulk_status_url.to_s).to_raise(Errno::ECONNREFUSED)
        ...
        context 'when the TPS endpoint fails' do
          before do
            stub_request(:get, tps_bulk_status_url.to_s).to_return(status: 500, body: 'This Broke')
          end
