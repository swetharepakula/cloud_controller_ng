module VCAP::CloudController::BrokerApiHelper
  ...
  def async_provision_service(status: 202)
    stub_request(:put, %r{broker-url/v2/service_instances/[[:alnum:]-]+}).
      to_return(status: status, body: { dashboard_url: 'https://your.service.com/dashboard' }.to_json.to_s)
  ...
  def provision_service(opts={})
    ...
    stub_request(:put, %r{broker-url/v2/service_instances/[[:alnum:]-]+}).
      to_return(status: return_code, body: { dashboard_url: 'https://your.service.com/dashboard' }.to_json.to_s)
